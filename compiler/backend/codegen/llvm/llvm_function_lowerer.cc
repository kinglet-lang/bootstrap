  ...
  bool lower(const KirFunction &fn, std::string *error) {
    ...
    for (std::size_t i = 0; i < linear_.size(); ++i) {
      ...
      case KirOpcode::NativeFsExists: {
        const int argc = instr->operands[0];
        if (argc != 1) {
          *error = "native_fs_exists expects exactly one argument";
          return false;
        }
        llvm::Value *path = pop_value(&stack, error, &type_stack);
        if (path == nullptr) {
      ...
      case KirOpcode::NativeFsReadBytes: {
        const int argc = instr->operands[0];
        if (argc != 1) {
          *error = "native_fs_read_bytes expects exactly one argument";
          return false;
        }
        llvm::Value *path = pop_value(&stack, error, &type_stack);
        if (path == nullptr) {
        ...
        llvm::Value *result = builder.CreateCall(rt_.native_fs_read_bytes, {path});
        push(result);
        temps[i] = result;
        break;
      }
      case KirOpcode::NativeFsWriteBytes: {
        const int argc = instr->operands[0];
        if (argc != 2) {
          *error = "native_fs_write_bytes expects exactly two arguments";
          return false;
        }
