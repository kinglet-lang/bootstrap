...
IntWidthInfo width_signed(int bits) {
  return IntWidthInfo{true, bits};
}
IntWidthInfo width_unsigned(int bits) {
  return IntWidthInfo{false, bits};
}

std::optional<IntWidthInfo> parse_canonical_int(std::string_view name) {
  if (name == "int8")
    return width_signed(8);
  if (name == "int16")
    return width_signed(16);
  if (name == "int32")
    return width_signed(32);
  if (name == "int64")
    return width_signed(64);
  if (name == "uint8")
    return width_unsigned(8);
  if (name == "uint16")
    return width_unsigned(16);
  if (name == "uint32")
    return width_unsigned(32);
  if (name == "uint64")
    return width_unsigned(64);
  return std::nullopt;
}
...
IntWidthInfo int_width_info(const Type &type) {
  if (type.kind == TypeKind::Char)
    return width_signed(8);
  if (auto info = parse_canonical_int(type.name))
    return *info;
  return width_signed(64);
}
...
std::optional<Type> try_promote_integer_binary(const Type &left, const Type &right,
  ...
  if (left_is_literal && !right_is_literal && is_integer_type(right))
    return right;
  if (right_is_literal && !left_is_literal && is_integer_type(left))
    return left;
  if (left_is_literal && right_is_literal && left.name == right.name)
    return left;
  if (left_is_literal && right_is_literal)
    return std::nullopt;
  if (left.name == right.name && is_integer_type(left) && is_integer_type(right))
    return left;
  return std::nullopt;
}

Type promote_float_binary(const Type &left, const Type &right) {
  if (float_is_64(left) || float_is_64(right))
    return make_float_type("float64");
